...
class SessionFilteredContributionStore implements ContributionStore {
  ...
  async listEntities(query?: ContributionQuery) {
    return this.inner.listEntities({ ...query, sessionId: this.sessionId });
  }
...
/** Build operation dependencies that use the same contribution store as session-scoped routes. */
export function operationDepsForSession(
  ...
  const scopedFrontier =
    sessionId !== undefined && deps.frontierForSession !== undefined
      ...
      : sessionId !== undefined
        ? new DefaultFrontierCalculator(scopedContributionStore)
        : deps.frontier;
