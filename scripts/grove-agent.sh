...
#
# Expects GROVE_DIR env var pointing to the .grove directory
# (set automatically by the TUI spawn manager).
...
# Resolve grove directory — GROVE_DIR is set by the TUI, fall back to detection
GROVE_FLAG=""
if [ -n "${GROVE_DIR:-}" ]; then
  GROVE_FLAG="--grove $GROVE_DIR"
fi
...
echo "  grove dir:  ${GROVE_DIR:-<auto-detect>}"
echo "  agent id:   $GROVE_AGENT_ID"

# Read current frontier to know what's available
FRONTIER=$($GROVE_BIN $GROVE_FLAG frontier --json 2>/dev/null || echo '[]')
LOG=$($GROVE_BIN $GROVE_FLAG log --limit 5 2>/dev/null || echo 'No contributions yet')
...
3. Submit your work by running:
   NEXUS_API_KEY=$NEXUS_API_KEY GROVE_AGENT_ID=$GROVE_AGENT_ID GROVE_AGENT_NAME=$GROVE_AGENT_NAME $GROVE_BIN $GROVE_FLAG contribute --kind work --summary '<describe what you did>' --mode evaluation --artifacts <your-file>
...
4. Submit your review by running:
   NEXUS_API_KEY=$NEXUS_API_KEY GROVE_AGENT_ID=$GROVE_AGENT_ID GROVE_AGENT_NAME=$GROVE_AGENT_NAME $GROVE_BIN $GROVE_FLAG review ${LATEST_WORK:-HEAD} --summary '<your review>' --score quality=<0.0-1.0>
