#!/bin/bash
# setup-lambda.sh — One-time setup for Cloud GPU (Lambda Cloud H100).
#
# Installs dependencies and prepares the original autoresearch repo.
# Idempotent — safe to run multiple times.
#
# Usage:
#   ./examples/real-autoresearch/setup-lambda.sh [--autoresearch-dir /path/to/autoresearch]

set -uo pipefail

AUTORESEARCH_DIR="${1:-${AUTORESEARCH_DIR:-../autoresearch}}"

echo "=== Cloud GPU (H100) Setup ==="
...
if [ -d "$AUTORESEARCH_DIR/data" ] && [ "$(ls -A "$AUTORESEARCH_DIR/data" 2>/dev/null)" ]; then
  ...
  echo "Preparing training data (one-time, ~5 min)..."
...
if command -v nvidia-smi >/dev/null 2>&1; then echo "  GPU:   OK ($(nvidia-smi --query-gpu=name --format=csv,noheader | head -1))"; else echo "  GPU:   WARNING — nvidia-smi not found"; fi
...
if [ "$local_ok" -eq 1 ]; then
  ...
  echo "  GROVE_NO_TRAIN_LOCK=1 NEXUS_PORT=1001 ../grove/examples/real-autoresearch/launch.sh"
else
