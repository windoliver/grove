...
4. Spawns enabled services in parallel -- HTTP server (port 4515) and, if the
...
Grove integrates with [Nexus](https://github.com/nexi-lab/nexus) as a shared
VFS backend for multi-agent coordination. Three connection modes are supported:
...
For Nexus development, build from a local source checkout instead of pulling
...
--compose-file ~/nexus/nexus-stack.yml` to `nexus up`. The repo-checkout compose
...
- `grove up`: `--headless` (CI mode, no TUI), `--no-tui` (server-only),
  `--nexus-source <path>` (build from source), `--build` (same, via `NEXUS_SOURCE` env)
